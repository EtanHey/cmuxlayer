...
describe("spawnDaemonProcess", () => {
  it("passes the pane capability to a detached daemon without exposing it in diagnostics", async () => {
    const root = mkdtempSync(join(tmpdir(), "cmuxlayer-capability-daemon-"));
    const token = "test-pane-capability";
    ...
    try {
      ...
      writeFileSync(daemonScriptPath,
        ...
        "setTimeout(() => process.stderr.write(process.env.CMUX_SOCKET_CAPABILITY.slice(8) + '\\n'), 10);\n");
