...
it("#641 confines actual child state writes to the benchmark root despite inherited production paths", async () => {
  ...
  const child = spawnSync(process.execPath, ["--import", "tsx", "--input-type=module", "-e", `
    ...
    paths.push(join(ctx.stateDir, 'events.jsonl'), state.stateFilePath('isolation-canary'), inboxPath('isolation-canary'), written.path, process.env.CMUXLAYER_BENCH_GH_RECEIPT);
    if (github !== null || !paths.every(existsSync)) throw new Error('missing contained writer artifact');
    if (!readFileSync(join(ctx.stateDir, 'events.jsonl'), 'utf8').includes('writeState')) throw new Error('missing real state event');
  ...
  expect(readFileSync(join(parentHome, "existing-data"), "utf8")).toBe("preserve seeded parent");
}, 25_000);
