...
// throwaway harness home and seed the sessions they claim to have captured.
...
describe("AgentEngine", () => {
  ...
  describe("spawnAgent", () => {
    ...
    it("assigns each managed surface a launcher-preserving unique seat title", async () => {
      ...
      // #479/#492: two workers in one repo must not share a pane title -- the
      // operator has to know which pane is which before closing one.
      expect(first.agent_id).not.toBe(second.agent_id);
      expect(mockClient.renameTab).toHaveBeenNthCalledWith(
        1,
        "surface:first-worker",
        `${first.agent_id} [surface:first-worker]`,
        { workspace: "ws:1" },
      );
      expect(mockClient.renameTab).toHaveBeenNthCalledWith(
        2,
        "surface:second-worker",
        `${second.agent_id} [surface:second-worker]`,
        { workspace: "ws:1" },
      );
    });
...
Session ID: ${sessionId}`,
    ...
    it("uses the saved launch cwd when capturing worktree transcript sessions", async () => {
      ...
      // The capture path prefers this override over HOME, and the file-level
      // harness home would otherwise shadow the transcript written above.
  ...
  describe("waitFor", () => {
    for (const cli of ["claude", "codex"] as const) {
      it(`WF3 does not fail fast on one resting ${cli} frame from a stale done record`, async () => {
        vi.useFakeTimers();
        try {
          const id = `wf3-${cli}`;
          const surface = `surface:${id}`;
          const started = Date.now();
          const active = cli === "codex"
            ? "OpenAI Codex\nWorking (2s • esc to interrupt)"
            : "Claude Code\n✻ Swirling… (2s · esc to interrupt)\n❯";
          const resting = cli === "codex" ? "OpenAI Codex\nModel: gpt-6-sol\n›" : "Claude Code\n❯";
          const frame = () => {
            const elapsed = Date.now() - started;
            return (elapsed >= 2_000 && elapsed < 3_000) || elapsed >= 4_000
              ? resting : active;
          };
          stateMgr.writeState(makeRecord({ agent_id: id, state: "done", surface_id: surface, cli, role: "worker" }));
          liveSurfaces = [makeSurface(surface)];
          (mockClient.readScreen as ReturnType<typeof vi.fn>).mockImplementation(async () => ({
            surface, text: frame(), lines: 80, scrollback_used: false,
          }));
          engine.setFreshLiveStateProbe(async (agent) =>
            resolveLiveAgentState(agent, parseScreen(frame())));
          await engine.getRegistry().reconstitute();
          let settled = false;
    ...
    it("WF3 confirms a done record with a genuinely resting screen promptly", async () => {
      vi.useFakeTimers();
      try {
        const id = "wf3-finished";
        const surface = `surface:${id}`;
        stateMgr.writeState(makeRecord({ agent_id: id, state: "done", surface_id: surface, cli: "codex", role: "worker" }));
        liveSurfaces = [makeSurface(surface)];
        (mockClient.readScreen as ReturnType<typeof vi.fn>).mockResolvedValue({
          surface, text: "OpenAI Codex\nModel: gpt-6-sol\n›", lines: 80, scrollback_used: false,
        });
        engine.setFreshLiveStateProbe(async (agent) =>
          resolveLiveAgentState(agent, parseScreen("OpenAI Codex\nModel: gpt-6-sol\n›")));
        await engine.getRegistry().reconstitute();
        const pending = engine.waitFor(id, "idle", 7_000);
        await vi.advanceTimersByTimeAsync(3_000);
        const result = await pending;
    ...
    it("WF3 vetoes stale done when a direct Codex frame is working at confirmation", async () => {
      vi.useFakeTimers();
      try {
        const id = "wf3-direct-working";
        const surface = `surface:${id}`;
        const started = Date.now();
        const resting = "OpenAI Codex\nModel: gpt-6-sol\n›";
        const active = "OpenAI Codex\nModel: gpt-6-sol\nWorking (2s • esc to interrupt)";
        stateMgr.writeState(makeRecord({ agent_id: id, state: "done", surface_id: surface, cli: "codex", role: "worker" }));
        liveSurfaces = [makeSurface(surface)];
        engine.setFreshLiveStateProbe(async (agent) =>
          resolveLiveAgentState(agent, parseScreen(Date.now() - started < 2_000 ? active : resting)));
        (mockClient.readScreen as ReturnType<typeof vi.fn>).mockImplementation(async () => ({
          surface, text: Date.now() - started < 4_000 ? active : resting,
          lines: 80, scrollback_used: false,
        }));
        await engine.getRegistry().reconstitute();
        let settled = false;
        const pending = engine.waitFor(id, "idle", 8_000);
        void pending.then(() => { settled = true; });
        await vi.advanceTimersByTimeAsync(3_500);
        expect(settled).toBe(false);
        await vi.advanceTimersByTimeAsync(3_500);
    ...
    it("WF3 timeout does not report done from one cached resting frame", async () => {
      vi.useFakeTimers();
      try {
        const id = "wf3-timeout";
        const surface = `surface:${id}`;
        stateMgr.writeState(makeRecord({ agent_id: id, state: "done", surface_id: surface, cli: "claude", role: "worker" }));
        liveSurfaces = [makeSurface(surface)];
        let probes = 0;
        engine.setFreshLiveStateProbe(async (agent) =>
          ++probes === 1
    ...
    it("WF3 also gates a done record written after wait entry", async () => {
      vi.useFakeTimers();
      try {
        const id = "wf3-late-done-record";
        const surface = `surface:${id}`;
        const started = Date.now();
        const active = "Claude Code\n✻ Swirling… (2s · esc to interrupt)\n❯";
        const resting = "Claude Code\n❯";
        const frame = () => {
          const elapsed = Date.now() - started;
          return (elapsed >= 2_000 && elapsed < 3_000) || elapsed >= 4_000
            ? resting : active;
        };
        stateMgr.writeState(makeRecord({ agent_id: id, state: "working", surface_id: surface, cli: "claude", role: "worker" }));
        liveSurfaces = [makeSurface(surface)];
        engine.setFreshLiveStateProbe(async (agent) =>
          resolveLiveAgentState(agent, parseScreen(frame())));
        (mockClient.readScreen as ReturnType<typeof vi.fn>).mockImplementation(async () => ({
        ...
        stateMgr.writeState(makeRecord({ agent_id: id, state: "done", surface_id: surface, cli: "claude", role: "worker", version: 1 }));
    ...
    it("WF3 preserves done when a finished CLI has fallen back to a shell", async () => {
      ...
      liveSurfaces = [makeSurface("surface:wf3-crashed")];
      engine.setFreshLiveStateProbe(async (agent) =>
    ...
    it("keeps a completed worker done when its live screen returns to ready", async () => {
      ...
      try {
        ...
        const readyPending = engine.waitFor("done-but-ready", "ready", 500);
        await vi.advanceTimersByTimeAsync(1_000);
  ...
  describe("shell-state CLI exit", () => {
    ...
    it("leaves an intentionally stopped agent stopped (#492)", async () => {
      ...
      stateMgr.writeState(
        makeRecord({
          ...
          user_killed: true,
          error: "Surface surface:cli-auto-revive-stopped disappeared",
        }),
      );
