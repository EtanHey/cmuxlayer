...
function boundedReportHeaderLines(headers: string[]): string[] {
  const listed = headers
    .slice(0, REPORT_NOTIFICATION_HEADER_LIMIT)
    .map((header) => {
  ...
  const omitted = headers.length - listed.length;
  ...
  const body = hasLostReportEntries
    ? [
        ...
        ...(event.report_lost_headers?.length
          ? [
              "Missing entries:",
              ...boundedReportHeaderLines(event.report_lost_headers),
            ]
          : []),
      ].join("\n")
    : hasReportBatch
      ? [
          `Watch ${event.watch_id} for ${event.owner}: report changed (${reportHeaders.length}); target=${event.target}`,
          "New entries:",
          ...boundedReportHeaderLines(reportHeaders),
        ].join("\n")
