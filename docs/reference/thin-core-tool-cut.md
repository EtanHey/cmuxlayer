...
The current surface registers a 10-tool public MCP palette. The remaining 22 internal definitions are not registered and are not ToolSearch-callable; `reorder_surface` is explicitly deleted.
...
The public registry has 10 names. Source retains 32 unique internal definitions; the drift guard checks the exact public names and both counts against current documentation in CI.
...
Only the exact 10-name public surface is callable; 22 internal definitions are unavailable through MCP. `reorder_surface` is absent.
